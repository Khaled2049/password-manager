 ...
 * via pre-signed URLs with ETag verification and optimistic concurrency control
 */
...
export interface UploadOptions {
  /** Expected ETag for optimistic locking (prevents concurrent overwrites) */
  ifMatch?: string;
  /** Abort signal for canceling the upload */
  signal?: AbortSignal;
...
export interface DownloadOptions {
  /** Expected ETag to verify data integrity (detects tampering) */
  expectedEtag?: string;
  /** Abort signal for canceling the download */
  signal?: AbortSignal;
...
export class S3Client {
   ...
   * @param options - Download options including ETag verification and progress tracking
   * @returns Object containing the downloaded data, ETag, and metadata
   * @throws S3ClientError if ETag mismatch, download fails, or network error occurs
   */
  async download(
    url: string,
